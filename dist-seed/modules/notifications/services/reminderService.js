...
exports.sendComplianceExpirationReminders = void 0;
const Compliance_1 = __importDefault(require("@/modules/compliance/models/Compliance")); // Corrected default import
const Employee_1 = __importDefault(require("@/modules/employees/models/Employee")); // Corrected default import
const User_1 = __importDefault(require("@/modules/auth/models/User")); // Corrected default import
const sendEmail_1 = require("@/lib/email/sendEmail");
const sequelize_1 = require("sequelize");
const date_fns_1 = require("date-fns"); // For formatting dates in emails
/**
 ...
 */
const sendComplianceExpirationReminders = async (daysUntilExpiry) => {
    console.log(`Checking for compliance items expiring in exactly ${daysUntilExpiry} days...`);
    const targetDate = new Date();
    targetDate.setDate(targetDate.getDate() + daysUntilExpiry);
    try {
        const expiringItems = await Compliance_1.default.findAll({
            where: {
                // Compare only the date part, ignoring time
                expirationDate: {
                    [sequelize_1.Op.eq]: (0, sequelize_1.literal)(`date('${targetDate.toISOString().split('T')[0]}')`),
                },
        ...
        for (const item of expiringItems) {
            ...
            const formattedExpiryDate = (0, date_fns_1.format)(new Date(item.expirationDate), 'MMMM d, yyyy');
...
exports.sendComplianceExpirationReminders = sendComplianceExpirationReminders;
